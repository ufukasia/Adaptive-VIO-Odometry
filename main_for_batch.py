...
# Sonuçları saklayacağımız CSV dosyasının adı
RESULTS_FILE = "run_results.csv"


def generate_run_id(config, sequence_name):
    """Config ve sekans isminden benzersiz bir ID üretmek için sıralı JSON kullan."""
    config_copy = config.copy()
    config_copy['activation_func'] = config['activation_func'].__name__
    config_copy['sequence_name'] = sequence_name  # Sekans ismini ekliyoruz

    config_str = json.dumps(config_copy, sort_keys=True)
    return hashlib.md5(config_str.encode()).hexdigest()
def check_existing_run(config, sequence_name):
    """Verilen config ve sequence_name ile run_results.csv'de kayıt olup olmadığını kontrol eder."""
    run_id = generate_run_id(config, sequence_name)
    
    # Eğer CSV dosyası yoksa doğrudan False döndür
    ...
    with open(RESULTS_FILE, 'r', newline='') as f:
        ...
        for row in reader:
            if row['run_id'] == run_id and row['sequence_name'] == sequence_name:
                print(f"Bu çalıştırma zaten kayıtlı: {sequence_name} (Run ID: {run_id})")
                return True  # Kayıt mevcut
    return False  # Kayıt bulunmadı



def save_results_to_csv(config, rmse_quaternions, rmse_euler_angles, sequence_name):
    run_id = generate_run_id(config, sequence_name)  # Sekans ismi burada kullanılıyor
    row = {
        'run_id': run_id,
        'sequence_name': sequence_name,  # Sekans ismi kayda ekleniyor
        'alpha': config['alpha'],
        'beta': config['beta'],
        'gamma': config['gamma'],
        'theta_threshold': config['theta_threshold'],
        'activation_function': config['activation_func'].__name__,
        'rmse_quaternions': list(rmse_quaternions),
        'rmse_euler_angles': list(rmse_euler_angles),
    }
        ...
        with open(RESULTS_FILE, 'r', newline='') as f:
            ...
            for existing_row in reader:
                if existing_row['run_id'] == run_id:
                    rows.append(row)  # Güncellenmiş satırı ekle
                else:
...
def main(args):
    ...
    if check_existing_run(config, args.sequence):
        print("Program sonlandırılıyor. Aynı konfigürasyon ve sekansla kayıt zaten mevcut.")
    ...
    # Sekans ismiyle birlikte sonuçları kaydet
    save_results_to_csv(config, rmse_quaternions, rmse_euler_angles, args.sequence)
