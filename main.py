...
def main(args):
    ...
    try:
        data, aligned_quaternions, aligned_euler_angles, true_quaternions, true_euler_angles, rmse_quaternions, rmse_euler_angles, thetas, timestamps = process_imu_data(
            str(imu_with_groundtruth_path), str(camera_file_path), str(camera_data_path), str(camera_yaml_path), config, args.sequence, verbose=True)
        print("Quaternion RMSE:", rmse_quaternions)
        print("Euler Angle RMSE:", rmse_euler_angles)
        
        # Use the dataset name for the CSV file
        ...
        visualize_results(data, aligned_quaternions, aligned_euler_angles, true_quaternions, true_euler_angles, rmse_quaternions, rmse_euler_angles, thetas, args.sequence)
        visualize_error(data, aligned_quaternions, aligned_euler_angles, true_quaternions, true_euler_angles, thetas, args.sequence)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--alpha", type=float, default=1.05, help="Alpha parameter (default: 1.05)")
    parser.add_argument("--beta", type=float, default=1.05, help="Beta parameter (default: 1.05)")
    parser.add_argument("--gamma", type=float, default=1, help="Gamma parameter (default: 1)")
    parser.add_argument("--theta_threshold", type=float, default=0.30, help="Theta threshold (default: 0.3)")
    parser.add_argument("--activation_function", type=str, 
                        choices=['quadratic_unit_step', 'cubic_unit_step', 'quartic_unit_step','relu', 'double_exponential_sigmoid', 'triple_exponential_sigmoid','quadruple_exponential_sigmoid', 'step'], 
